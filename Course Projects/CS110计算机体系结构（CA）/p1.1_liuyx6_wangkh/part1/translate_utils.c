...
{
    /* check if the input pointers str and output are valid */
    long int res;   
    if(!output||!str) return -1;
    /*convert a string to a long int value*/
    res = strtol(str, NULL, 0);
    /*check if the res value result falls within the range of lower_bound and upper_bound. */
    if(res<=upper_bound && res>=lower_bound){
        /*store the res into the memory location pointed by output*/
        *output = res;
        return 0;
    }
    return -1;
...
int translate_reg(const char *str)
{
    ...
    /*Translation for the Temporaries registers t0-t2,s0-s1,a0-a1*/
    ...
    /*Translation for the Function Arguments registers a2-a7*/
    ...
    /*Translation for the saved registers s2-s7*/
    ...
    /*Translation for the saved registers s8-s11*/
    ...
    /*Translation for the Temporaries registers t3-t6*/
