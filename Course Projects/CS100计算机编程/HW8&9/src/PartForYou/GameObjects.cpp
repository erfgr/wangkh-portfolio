...
GameWorld* GameObject::GetLevel() const {
  return m_world;
}
...
Explosion::Explosion(int x, int y, double size, GameWorld* world)
  : BackGround(IMGID_EXPLOSION, x, y, 0, 3, size, world) {}
...
Stone::Stone(int x, int y, double size, GameWorld* world)
  : BackGround(IMGID_METEOR, x, y, 0, 1, 2.0, world) {}

Stone::~Stone() {}

void Stone::UpdateActions() {
  MoveTo(GetX(), GetY() - 1);
}
...
void Dawnbreaker::UpdateActions() {
  ...
  if (GetWorld()->GetKey(KeyCode::FIRE1)) {
    if (m_energy >= 10) {
      m_energy -= 10;
      GetWorld()->Instantiate(new BlueBullet(GetX(), GetY() + 50, BASE_ATTACK+3*m_level, GetWorld(), BASE_BULLET_SIZE+0.1*m_level));
    }
  }
  if (GetWorld()->GetKeyDown(KeyCode::FIRE2)) {
    if (m_energy >= 1) {
      m_energy -= 1;
      GetWorld()->Instantiate(new Stone(GetX(), GetY() + 100,2.00, GetWorld()));
    }
  }
